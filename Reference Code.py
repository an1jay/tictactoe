...
class QLearnerPlayer(Player):
    ...
    def reward(self, value, board):
        ...
        if self.prevMove:
            ...
            maxQnew = max(
                [
                    self.getQ(tuple(board), a)
                    for a in self.availableMoves(self.prevBoard)
                ]
            )

            self.q.fit(
                self.vectoriseSA(self.prevBoard, self.prevMove),
...
if __name__ == "__main__":
    ...
    for i in range(trainCycles):
        if i % 50 == 0:
            print(
                "{0:.1f}%, ".format(i * 100 / trainCycles)
                + str(int(time.time() - t0))
                + "s",
                end=" |\n",
            )
